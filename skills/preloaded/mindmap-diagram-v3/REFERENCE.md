...
| 交付物 | 文件格式 | 用途 |
|---|---|---|
| 可编辑源文件 | `.mmd`（Mermaid） | 后续修改、二次编辑、版本控制 |
| 渲染图片 | `.png` / `.svg` / `.html` | 查看、分享、嵌入文档 |

### 渲染方式选择

| 环境 | 推荐渲染方式 | 输出格式 |
|---|---|---|
| 已安装 mmdc | `mmdc -i <name>.mmd -o <name>.png` | PNG/SVG |
| 已安装 mddoc-cli | `mddoc mindmap <name>.mmd` | PNG |
| 无额外工具 | `python scripts/render_diagram.py <name>.mmd` | HTML |
| 在线环境 | [Mermaid Live Editor](https://mermaid.live) | PNG/SVG |

### 脑图专用输出

脑图除了 Mermaid 源文件，还支持以下可编辑格式：

| 格式 | 扩展名 | 编辑工具 | 说明 |
|---|---|---|---|
| Markdown 大纲 | `.md` | 任何文本编辑器 | # 层级结构，可导入 XMind |
| Markmap 格式 | `.mmd` | markmap-lib / mddoc | Markdown + mindmap 语法 |
| XMind 文件 | `.xmind` | XMind | 需手动导入 Markdown 大纲 |
...
├── auth-flow.png           # 渲染图片（查看/分享）
├── module-overview.mmd     # 另一张图的源文件
├── module-overview.png     # 另一张图的渲染图片
├── project-summary.md      # 脑图的 Markdown 大纲（可导入 XMind）
└── project-summary.png     # 脑图的渲染图片
```
