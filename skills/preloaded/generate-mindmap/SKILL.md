...
**当 JSON 数据包含 emoji、中文、嵌套结构等复杂内容时，必须使用 `input`（stdin）参数传入，不要通过 `--data` CLI 参数传入。** CLI 参数传复杂 JSON 容易因 shell 转义/截断导致解析失败。

正确用法（使用 input/stdin）：
```
execute_skill_script(
  skill_name="mindmap",
  script_path="generate_mindmap.py",
  args=["--title", "中期小结", "--format", "html", "--output", "midterm_mindmap.html"],
  input="JSON数据字符串"
)
```
...
execute_skill_script(
  skill_name="mindmap",
  script_path="generate_mindmap.py",
  args=["--title", "简单主题", "--format", "html", "--data", "简单JSON"]
)
```
...
> **Sandbox 环境下 Pillow 已预装到 Docker 镜像中，无需手动安装。** 脚本不会在运行时尝试 pip install。
