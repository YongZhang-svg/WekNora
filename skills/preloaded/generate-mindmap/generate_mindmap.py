...
def _ensure_pillow():
    """Try to import Pillow; if missing, report error (no auto-install in sandbox)."""
    try:
        from PIL import Image  # noqa: F401
        return True
    except ImportError:
        pass
    print("[mindmap] ERROR: Pillow is not installed. It must be pre-installed in the sandbox image.", file=sys.stderr)
    print("    Add 'pillow' to the pip install step in Dockerfile.sandbox", file=sys.stderr)
    return False
...
def resolve_output(raw_path: str, fmt: str) -> str:
    ...
    it is placed under ~/workspace/ instead of the current dir.
    """
...
def parse_args():
    ...
    p.add_argument("--data",    default=None,   help="JSON string describing the mind map structure. If omitted, reads from stdin.")
    p.add_argument("--stdin",   action="store_true", help="Read JSON data from stdin instead of --data argument")
...
def main():
    ...
    # Resolve data source: --data arg > stdin > error
    data_str = args.data
    if not data_str:
        if not sys.stdin.isatty() or args.stdin:
            data_str = sys.stdin.read()
            if not data_str.strip():
                print("[mindmap] ERROR: No data provided — use --data or pipe JSON via stdin", file=sys.stderr)
                sys.exit(1)
        else:
            print("[mindmap] ERROR: --data is required when no stdin input is provided", file=sys.stderr)
            sys.exit(1)
